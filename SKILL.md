...
Turn any sufficiently source-backed content into a dated publishing package with Chinese and English NotebookLM media, image variants, and channel-ready copy. NotebookLM is an automation step, not a manual handoff: use browser automation to create a clean notebook, upload the intended PDF/source, generate media, download results, and verify disk files before claiming completion.
...
3. If the package depends on NotebookLM, automatically create or claim a clean notebook and upload only the intended source document(s). Visible counts such as `2 个来源`, `5 个来源`, or unexpected old titles are contamination until disproved.
4. If NotebookLM automation is blocked by login, permission, CAPTCHA, file chooser failure, UI changes, or generation still running, produce the source archive, local infographic fallback, publish copy, and an explicit blocker report instead of inventing media status.
...
3. Choose the content strategy.
   ...
   - If the preflight returns multiple strong angles, surface one primary-angle choice to the user before final drafts instead of guessing.
...
4. Build the NotebookLM inputs.
   ...
   - Use browser automation, preferably the user's logged-in Chrome session, to create a new clean notebook and upload the intended source document(s).
   - Do not stop at "manual upload needed" unless automation is genuinely blocked.
   ...
   - For automated upload/generation/download handling, read `references/notebooklm-automation.md`.
5. Download or create media.
   ...
   - For long reports, let NotebookLM video generation run for about 15 minutes before treating it as stuck. Continue with image, copy, and audit tasks while waiting.
   - Trigger NotebookLM video and infographic generation automatically after source verification; local fallbacks are parallel work, not a replacement for the automated NotebookLM attempt.
   - If the browser download event times out, inspect `~/Downloads` by modification time and verify the downloaded MP4/PNG before declaring the download failed.
   - Clearly distinguish official NotebookLM downloads from local fallback or placeholder assets in the execution note.
...
7. Validate the final package.
   ...
   - When official NotebookLM media is required, add `--expect-official-notebooklm video_zh`, `--expect-official-notebooklm video_en`, or `--expect-official-notebooklm infographic`.
...
# Then use browser automation per references/notebooklm-automation.md to upload/generate/download.
...
7. Do not treat a local fallback, placeholder, or post-processed asset as an official NotebookLM download unless the execution note and audit evidence both support it.
8. If NotebookLM is still generating in a clean notebook, keep the tab as the handoff state and report the exact visible status instead of rebuilding from scratch.
9. Do not end a package with manual NotebookLM instructions when browser automation is available and the user authorized NotebookLM upload/generation.

## Resources

- `scripts/audit_publish_pack.py`: audit deliverables, media dimensions, obvious missing files, optional expected terms, and verified NotebookLM-origin assets.
...
- `references/notebooklm-automation.md`: automated browser workflow for creating clean NotebookLM notebooks, uploading source PDFs, generating media, polling, downloading, and recording evidence.
